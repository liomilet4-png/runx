...
const specContents = String(inputs.spec_contents || "");
if (!specContents) {
  throw new Error("spec_contents is required.");
}
...
const files = [...declared.entries()]
  ...
  .map(([relativePath, declaredIn]) => {
    ...
    return {
      ...
      exists,
      kind:
        relativePath.startsWith(".ai/specs/") || relativePath.startsWith(".ai/reviews/")
          ? "governance_artifact"
          : "repo_file",
