...
async function assertCompletedRustOwner(): Promise<void> {
  ...
  for (const required of ["CatalogAdapter", "fixtures/runtime/adapters/catalog", "oracle_text"]) {
    if (!rustTest.includes(required)) {
      throw new Error(`${owner.rustTest} is missing Rust catalog ownership marker ${required}.`);
    }
  }
}
...
async function checkNoStaleOracleFiles(): Promise<void> {
  ...
  for (const filePath of await collectFiles(oracleRoot)) {
    if (!expectedOracleFiles.has(filePath)) {
      throw new Error(`stale runtime catalog adapter oracle file: ${relative(filePath)}`);
    }
  }
}
