...
async function assertCompletedRustOwner(): Promise<void> {
  ...
  for (const required of ["AgentAdapter", "RecordingResolver", "run_harness_fixture_with_adapter"]) {
    if (!rustTest.includes(required)) {
      throw new Error(`${owner.rustTest} is missing Rust agent ownership marker ${required}.`);
    }
  }
}
...
async function checkNoStaleOracleFiles(): Promise<void> {
  ...
  for (const filePath of await collectFiles(oracleRoot)) {
    if (!expectedOracleFiles.has(filePath)) {
      throw new Error(`stale agent adapter oracle file: ${relative(filePath)}`);
    }
  }
}
